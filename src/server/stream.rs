...
fn chat_stream(
    ...
    stream_response(output, headers)
}
    ...
    let output = stream! {
        ...
        while let Some(chunk) = source.next().await {
            ...
            match events {
                ...
                Err(error) => {
                    yield Ok::<_, Infallible>(render_anthropic_error(&error).into_bytes());
                    failed = true;
                    break;
                }
            }
    ...
    let output = stream! {
        ...
        while let Some(chunk) = source.next().await {
            ...
            match events {
                ...
                Err(error) => {
                    yield Ok::<_, Infallible>(gemini::SseEvent::named(
                        "error",
...
fn stream_response<S>(output: S, headers: HeaderMap) -> Response
where
...
fn sse_headers(source: &HeaderMap) -> HeaderMap {
    ...
    {
        headers.insert("x-codex-turn-state", turn_state);
    }
    headers
}
