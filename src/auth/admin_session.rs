...
use super::{constant_time_equal, sign_json, verify_json};
...
pub fn create_admin_session(admin_secret: &str, now_ms: i64) -> AppResult<String> {
    ...
    sign_json(&value, admin_secret, ADMIN_SESSION_PURPOSE).map_err(|_| unavailable())
}
...
pub fn has_valid_admin_session(
    ...
    let Ok(value) = verify_json(&token, admin_secret, ADMIN_SESSION_PURPOSE) else {
        return false;
    };
...
mod tests {
    ...
    fn expires_and_rotates_sessions() {
        ...
        let token = create_admin_session("admin-secret", now).unwrap();
        let cookie = admin_session_cookie_header(&token);
        assert!(has_valid_admin_session(Some(&cookie), "admin-secret", now));
        assert!(!has_valid_admin_session(Some(&cookie), "rotated", now));
